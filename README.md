...
python -m scripts.convert_data \
    --raw_path data/raw/<your dataset> \
    --out_path data/processed/<your dataset>

# Should give
tree data/processed/<your dataset>
├── test.json
├── train.json
└── val.json
```
...
python -m simplepatho.corpus_statistics \
    --data_path data/raw/<your dataset> \
...
In the paper, we run experiments with three models: Bert2Bert, Bert2Share, and mBART. Please refer to each script for training/inference parameters and how to use your own dataset.
...
python -m simplepatho.evaluate \
    --orig_path data/processed/<your dataset>/test.source \
    --refs_path data/processed/<your dataset>/test.target \
    --sys_paths \
        data/processed/<your dataset>/test.source \
        output/<your dataset>/bert2bert/predictions_test.txt \
        output/<your dataset>/bert2bert-shared/predictions_test.txt \
        output/<your dataset>/mbart/predictions_test.txt \
    --compact \
...
|_Reproduction of (Devaraj et al. 2021) with our code._|
